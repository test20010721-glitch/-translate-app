...
const s = {
  page: { minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, background: '#f5f5f0' },
  ...
  input: { width: '100%', fontSize: 15, padding: '10px 12px', borderRadius: 8, border: '1px solid #e0e0d8', fontFamily: 'inherit', boxSizing: 'border-box', marginBottom: 16 },
  btn: (loading) => ({ width: '100%', fontSize: 14, fontWeight: 600, padding: '11px', borderRadius: 8, border: 'none', background: loading ? '#ccc' : '#1a1a1a', color: '#fff', cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'inherit', marginTop: 4 }),
...
export default function LoginPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result?.error) { setError('メールまたはパスワードが間違っています'); return }
      router.push('/')
    } catch(e) {
      setError('エラーが発生しました')
    } finally {
